  ...
  <Card>
    ...
    >
      <div class="flex-1">
        <div
          :class="[
            'inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium uppercase tracking-[0.18em]',
            props.taskStatusClasses[props.task.status],
          ]"
        >
        ...
        <p class="mt-1 text-sm text-slate-500">
          Created by: {{ props.task.user_name ?? 'Unknown user' }}
        </p>
        <p v-if="props.task.description" class="mt-2 text-slate-600">
          {{ props.task.description }}
        </p>
        <div class="mt-4 flex flex-wrap items-center gap-4">
          <span class="text-sm text-slate-500"
            >Due: {{ props.formatDate(props.task.due_date) }}</span
          >
        </div>
      </div>
